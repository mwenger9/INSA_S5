...
add_stone_in_list(stone(X,Y),[X|R],Res):-
    append([Y],[X|R],Res).

add_stone_in_list(stone(X,Y),[Y|R],Res):-
    append([X],[Y|R],Res).

add_stone_in_list(stone(X,Y),[E|R],Res):-
    \==(X,E),
    \==(Y,E),
    add_stone_in_list(stone(X,Y),R,Res2),
    append([E],Res2,Res).


add_stone(stone(X,Y),chain([],[]),chain([X],[Y])).
add_stone(stone(X,Y),chain(L1,L2),chain(Res,L2)):-
    add_stone_in_list(stone(X,Y),L1,Res),
    !.

add_stone(stone(X,Y),chain(L1,L2),chain(L1,Res)):-
    add_stone_in_list(stone(X,Y),L2,Res),
    !.


chains([],_,Chains).
chains(Stones,PartialAcc,Chains):-
    choose(Stones,Elt,Reste),
